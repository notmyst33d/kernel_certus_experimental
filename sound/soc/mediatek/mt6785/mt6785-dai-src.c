				 ...
				 struct snd_soc_dai *dai)
{
	...
	unsigned int rate_reg = mt6785_rate_transform(afe->dev, rate, id);
...
int mt6785_dai_src_register(struct mtk_base_afe *afe)
{
	...
	/* set dai priv */
	ret = mt6785_dai_set_priv(afe, MT6785_DAI_SRC_1,
				  sizeof(struct mtk_afe_src_priv), NULL);
	if (ret)
		return ret;

	ret = mt6785_dai_set_priv(afe, MT6785_DAI_SRC_2,
				  sizeof(struct mtk_afe_src_priv), NULL);
	if (ret)
		return ret;
