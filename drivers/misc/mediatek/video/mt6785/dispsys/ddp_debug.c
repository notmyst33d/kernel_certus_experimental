...
void _ddic_test_write(void)
{
	...
	cmd_tab.dtype = 0x51;
	cmd_tab.dlen = 2;
	cmd_tab.payload = vmalloc(2 * sizeof(unsigned char));
	if (cmd_tab.payload == NULL) {
		DDPMSG("[DISP]%s: vmalloc fail.\n", __func__);
		return;
	}
	cmd_tab.payload[0] = 0x02;
	cmd_tab.payload[1] = 0x03;
	cmd_tab.vc = 0;
	cmd_tab.link_state = 1;

	do_lcm_vdo_lp_write(&cmd_tab, 1);
	DDPMSG("write lcm addr:0x%x, len:%d, val:0x%x,0x%x\n",
		cmd_tab.dtype, cmd_tab.dlen,
		*cmd_tab.payload, *(cmd_tab.payload + 1));
...
void _ddic_test_read_write(void)
{
	...
	//write display brightness
	write_tab.dtype = 0x51;
	write_tab.dlen = 2;
	write_tab.payload = vmalloc(2 * sizeof(unsigned char));
	if (write_tab.payload == NULL) {
		DDPMSG("[DISP]%s: vmalloc fail.\n", __func__);
		return;
	}
	write_tab.payload[0] = 0x02;
	write_tab.payload[1] = 0x03;
	write_tab.vc = 0;
	...
	DDPMSG("write lcm addr:0x%x, len:%d, val:0x%x,0x%x\n",
		write_tab.dtype, write_tab.dlen,
		*write_tab.payload, *(write_tab.payload + 1));

	DDPMSG("read lcm addr:0x%x, len:%d, val:0x%x,0x%x\n",
		read_tab.dtype, read_tab.dlen,
		*read_tab.payload, *(read_tab.payload + 1));
